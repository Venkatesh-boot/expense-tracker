  ...
  return (
    <div>
      ...
      <div className="overflow-x-auto rounded-lg shadow-lg border border-gray-200">
        <table {...getTableProps()} className="min-w-full text-left bg-white">
              ...
              <tr {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.map((column: any) => (
                  <th
                    ...
                    className="py-3 px-5 border-b font-semibold text-blue-700 text-base cursor-pointer select-none whitespace-nowrap"
                  >
            ...
            {page.map((row: any, idx: number) => {
              ...
              return (
                <tr
                  {...row.getRowProps()}
                  className={
                    `transition-colors ${idx % 2 === 0 ? 'bg-blue-50' : 'bg-white'} hover:bg-blue-100`}
                >
                  {row.cells.map((cell: any) => (
                    <td className="py-3 px-5 border-b text-gray-700 whitespace-nowrap" {...cell.getCellProps()}>
                      {cell.render('Cell')}
                    </td>
                  ))}
                </tr>
